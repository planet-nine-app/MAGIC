...
import resolveTest from './src/commands/resolve-test.js';
...
program
  .command('resolve')
  .description('Runs a single test against a single language/color resolver')
  .option('-c, --color <color>')
  .option('-l, --language <language>')
  .action((options) => {
    const color = options.color || config.languages[options.language];
    ...
    resolveTest(color, options.language);
  });
