(() => {
  ...
  // entry.js
  var utf8ToBytes3 = (str) => Uint8Array.from(Array.from(str).map((ch) => ch.charCodeAt(0)));
  var generateKeys = (privateKeyHex) => {
    const publicKey = bytesToHex(secp256k1.getPublicKey(privateKeyHex));
    return { privateKey: privateKeyHex, publicKey };
  };
  var sign = (message, privateKeyHex) => {
    const messageHash = keccak256(utf8ToBytes3(message));
    const signature = secp256k1.sign(messageHash, privateKeyHex);
    return signature.toCompactHex();
  };
  var verifySignature = (sig, message, pubKeyHex) => {
    const messageHash = keccak256(utf8ToBytes3(message));
