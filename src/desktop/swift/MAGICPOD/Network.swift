...
struct Registration {
    ...
    func toString() -> String {
        return """
        {"timestamp":"\(timestamp)","pubKey":"\(pubKey)\",\"signature\":\"\(signature)\",\"hash\":\"\(hash)\"}
        """
    }
}
...
class Network {
    ...
    class func register(payload: Registration, callback: @escaping (Error?, Data?) -> Void) async {
        guard let data = payload.toString().data(using: .utf8) else { return }
        await Network.put(urlString: "\(baseURL)user/create", payload: data) { err, data in
            if let err = err {
                print(err)
                print("errororororo")
            }
