...
void webSocketEvent(WStype_t type, uint8_t * payload, size_t length) {
    switch(type) {
        ...
        case WStype_CONNECTED:
            ...
            webSocket.sendTXT("{\"type\":\"gateway_connected\",\"device\":\"um_pros3\"}");
            break;
...
void setup() {
    ...
    // Initialize RGB LED and set to purple
    pixel.begin();
    pixel.setPixelColor(0, pixel.Color(128, 0, 128));  // Purple (R, G, B)
    pixel.show();

    Serial.println("\n\n================================");
    Serial.println("LoRa Gateway - UM Pro S3");
