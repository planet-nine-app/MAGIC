...
console.log(req.body);
  const resolution = await superagent.post('http://localhost:3002/resolve')
    .send(req.body)
...
app.post('/spell', async (req, res) => {
  ...
  console.log(chalk.cyan('Received Spell:'));
  console.log(req.body);
  ...
  try {
    const nextPath = 'http://localhost:3002/register';
    ...
    console.log(chalk.cyan('Added to spell:'));
    console.log(spell);
...
console.log(resolution.body);
    ...
    return res.send(resolution.body);
  } catch(err) {
